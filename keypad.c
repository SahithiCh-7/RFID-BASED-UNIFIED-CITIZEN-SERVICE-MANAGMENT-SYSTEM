...
u8 KeyScanWithTimeout(u32 timeout_ms)
{
	...
	while (ColScan())
	{
		...
		{
			u32 remaining_seconds = (timeout_ms - elapsed) / 1000;
			if (remaining_seconds != last_timer_sec)
			{
				extern volatile u8 lcd_current_pos;
				u8 saved_pos = lcd_current_pos;
				...
				CmdLCD(saved_pos); // Restore cursor position
				
				elapsed += 12; // Compensate for LCD write delay (6 writes * 2ms = 12ms)
			}
		}
